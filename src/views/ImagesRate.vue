    ...
    <div>
        ...
        <div v-if="isInitiated">
            <div class="menu1">
                ...
                <button v-shortkey.once="['.']" @shortkey="nextSlide()" @click="nextSlide()">&gt;</button>
            </div>
            <div class="menu2">
                <button
                    v-for="i in 4"
                    v-bind:key="i"
                    v-shortkey.once="[i]" @shortkey="rateImages(i)" @click="rateImages(i); btnCounter++;"
                    :class="{active: activeScore == i}"
                >{{ i }}</button>
                <div>
                <span>Buttons pressed: {{btnCounter}} </span>
                </div>
                <div>
            ...
            <div v-for="(imagesGroup, idx) in groupArray(imagesData, 3)" v-bind:key="idx">
                <div class="row" >
                    <div class="column" v-for="imageData in imagesGroup" v-bind:key="imageData.postId">
...
export default {
    ...
    methods: {
        ...
        getAndPreload(type, onComplete) {
            ...
            Axios
                .post(config.apiUrl('get'+type+'Images'), { refId: this.imagesData.length > 0 ? this.imagesData[0].authorId : 0 })
                .then(res => {
        ...
        updateSlide(dryShot) {
            if (!dryShot) {
                ...
                this.profileCounter = this.profileCounter + this.nextImagesData.length;
            }
